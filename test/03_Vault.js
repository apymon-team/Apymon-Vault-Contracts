...
describe("Vault", function () {
	...
	describe("Withdraws", function () {
		...
		it("Withdrawing multiple tokens emits multiple events", async function () {
			...
			await expect(
				vaultProxy.connect(userA).withdrawMultiple([ethWithdraw, erc20Withdraw, erc721Withdraw, erc1155Withdraw, cryptopunkWithdraw], userB.address)
			)
				.to.emit(vaultProxy, 'WithdrawETH').withArgs(userA.address, userB.address, ethers.utils.parseEther('1', 'ether'))
				.to.emit(vaultProxy, 'WithdrawERC20').withArgs(userA.address, erc20Token.address, userB.address, 1)
				.to.emit(vaultProxy, 'WithdrawERC721').withArgs(userA.address, erc721Token.address, 1, userB.address)
				.to.emit(vaultProxy, 'WithdrawERC1155').withArgs(userA.address, erc1155Token.address, 1, userB.address, 1)
				.to.emit(vaultProxy, 'WithdrawCryptoPunk').withArgs(userA.address, cryptoPunksToken.address, 1, userB.address);
